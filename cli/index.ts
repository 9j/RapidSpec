...
const TECH_STACKS = {
	...
	"nextjs-only": {
		...
		agents: ["nextjs-architecture-expert.md"],
		includeDatabase: false,
	},
};
...
function filterContentByStack(content: string, stackConfig: any): string {
	if (stackConfig.includeDatabase) {
		...
		filtered = filtered.replace(/### Database.*?### \w+/gs, (match) => {
			if (
				match.includes("Database Migration") ||
				match.includes("Database Schema")
			) {
				return "";
			}
			return match;
...
program
	...
	.option(
		"--stack <stack>",
		"Technology stack (none, nextjs-supabase, nextjs-only)",
	)
	.action(async (targetPath, options) => {
		...
		if (!stackConfig) {
			...
			console.log(
				chalk.gray("Available stacks:", Object.keys(TECH_STACKS).join(", ")),
			);
			process.exit(1);
		}
			...
			const agentFiles = allAgentFiles.filter((file) => {
				...
				if (selectedStack === "none") {
					return (
						...
						file.includes("code-reviewer") ||
						file.includes("architecture") ||
						file.includes("best-practices")
					);
				}
		...
		if (existsSync(templatesSrcDir)) {
			const templateFiles = readdirSync(templatesSrcDir).filter(
				(f: string) =>
		...
		console.log(
			chalk.gray("     Reload Cursor to load RapidSpec agents via MCP"),
		);
		console.log(
			chalk.gray("     Ask naturally: 'Use the code-verifier agent to...'"),
		);
		console.log(
			chalk.gray("     Commands: /rapid:* slash commands work in Cursor 2.0\n"),
		);
